...
void InstanceContext::UpdateSceneData()
{
	for (auto& ritem : m_AllRitems)
	{
		//const auto& instanceData = ritem->m_Instances;
		//int visibleInstanceCount = 0;

		//for (UINT i = 0; i < (UINT)instanceData.size(); ++i)
		//{
		//	auto instanceItem = instanceData[i];
		//	XMMATRIX world = XMLoadFloat4x4(&instanceItem->World);
		//	XMMATRIX texTransform = XMLoadFloat4x4(&instanceItem->TexTransform);
		...
		//		data.MaterialIndex = instanceItem->MaterialIndex;
		//		// Write the instance data to structured buffer for the visible objects.
		//		currInstanceBuffer->CopyData(visibleInstanceCount++, data);
		//	}
		//}
		//ritem->InstanceCount = visibleInstanceCount;
